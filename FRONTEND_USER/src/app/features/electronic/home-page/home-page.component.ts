...
export class HomePageComponent {
  ...
  listCategory: any[] = [];
  listProduct: any[] = [];
  listFeaturedProd: any[] = []
  listBestReviewProduct: any[]=[]
  ...
  fecthListFeaturedProd(): void {
    this.productService.getAll().subscribe((res)=> {
      if(res.code === 200) {
        ...
        if(res.data.data) {
          this.listProduct.sort((a: any, b: any) => (b.visitCount > a.visitCount ? 1 : -1));
          this.listFeaturedProd = [...this.listProduct.slice(0,4)];
        }
      }
    })
    this.productService.getAll().subscribe((res)=> {
      if(res.code === 200) {
        ...
        if(res.data.data) {
          this.listProduct.sort((a: any, b: any) => (b.rating > a.rating ? 1 : -1));
          this.listBestReviewProduct = [...this.listProduct.slice(0,4)];
        }
      }
    })
  }
