...
export class AccountDetailComponent implements OnInit{
  ...
  cancelOrder(item: any) {
    this.modal.confirm({
      nzTitle: 'Bạn có chắc chắn muốn hủy đơn hàng không?',
      nzOnOk: () => {
        let data = {
          id: item.id,
          status: -1,
    ...
    this.cusService.update(model).subscribe(
      (res) => {
        if (res.code === 200) {
          ...
          this.nzMessage.error('Update account unsuccess');
        }
      },
      (err) => {
        this.nzMessage.error('Update account unsuccess');
  ...
  handleChange(info: NzUploadChangeParam): void {
    ...
    if (info.file.status === 'done') {
      this.nzMessage.success('Upload avatar success');
    } else if (info.file.status === 'error') {
      this.nzMessage.error('Upload avatar unsuccess');
  ...
  save() {
    ...
    this.cusService.changePassword(UpdateUserModel).subscribe(
      (res) => {
        if (res.code !== 200) {
          this.nzMessage.error(`Failed: ${res.error.message}`)
          return;
        }
        this.resetData();
        this.nzMessage.success('Change password success')
      },
      (err) => {
        console.log(err);
        this.nzMessage.error(`Failed: ${err.error.message}`)
