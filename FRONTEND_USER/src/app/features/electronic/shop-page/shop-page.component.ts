...
export class ShopPageComponent {
  ...
  fecthlistCategory(): void {
    this.categoryService.getListComboboxNoParent().subscribe(
      (res: any) => {
  ...
  searchBySuplier(item: any){
    ...
    this.fetchListProduct(this.filter);
    $('.brand').removeClass('active')
    $(`#brand-${item.name}`).addClass('active')
  }
  ...
  priceRange(): void {
    this.productService.getAll().subscribe((res: any) => {
      ...
      this.listProduct = this.listProduct.filter(item => item.amoutDefault >= this.fromPrice && item.amoutDefault <=this.toPrice)
