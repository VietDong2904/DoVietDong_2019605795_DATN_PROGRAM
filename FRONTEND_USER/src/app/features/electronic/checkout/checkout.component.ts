...
export class CheckoutComponent {
  ...
  ngOnInit(): void {
    ...
    this.cartService.currentCart.subscribe((res) => {
      ...
      this.listCart.map((item) => {
        ...
        item.productId = item.productColors[0].productId;
        item.sizeId = item.productColors[0].sizeId;
  ...
  save() {
    ...
    if (this.form.invalid) {
      this.nzMessage.error( 'Check all field you filled' );
      return;
    }
    if (this.paymentType === -1) {
      this.nzMessage.error('Choose the way payment');
    ...
    this.orderService.create(data).subscribe((res) => {
      if (res.code === 200) {
        this.nzMessage.success('Order successfull');
