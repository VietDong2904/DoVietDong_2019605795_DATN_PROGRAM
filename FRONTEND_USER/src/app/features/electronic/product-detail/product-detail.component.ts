...
export class ProductDetailComponent {
  ...
  myThumbnail: any;
  itemQuickView: any = {};
  tooltips = ['Very bad', 'Bad', 'Normal', 'Good', 'Awesome'];
  value = 5;
    ...
    if (code) {
      this.prodService.getById(code).subscribe(
        (res) => {
          ...
          this.productQuantity = this.itemQuickView.quantity
      ...
      this.prodService.getListSimilar(code).subscribe(
        (res) => {
          if (res.code === 200) {
            ...
            this.listSimilarActive =  this.listSimilarActive.slice(0,4)
  ...
  changeColor(item:any){
    ...
    this.itemQuickView.quantity = item.quantity;
  ...
  addToCart(item: any) {
    console.log(item);
    if(!this.chooseSize) {
      this.nzMessage.warning('Choose size please ');
    }
    else {
      console.log(this.itemQuickView.productColors)
    ...
    if (token) {
      ...
      switch (type) {
        case 1:
          if (this.inputReview === '' || this.inputReview === undefined || this.inputReview === null) {
            this.nzMessage.warning('Fill your comment, please!')
      ...
      this.prodReviewService.create(model).subscribe(
        (res) => {
          if (res.code === 200) {
            if (type === 1) {
              this.nzMessage.success('Thanks for reviewing. Have a nice day!')
            }
            this.fetchListProdReview();
            this.inputValue = '';
            this.inputReview = '';
        ...
        (err) => {
          this.nzMessage.error('Have some error ocurs: '+ err.error.message )
        },
      );
      this.prodService.getById(this.itemQuickView.id).subscribe(
        (res) => {
          this.itemQuickView.rating=res.data.rating
        },
        (err) => {
          console.log(err);
        },
      ...
      this.nzMessage.error('Login to comment');
